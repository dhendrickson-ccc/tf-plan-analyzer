...
def get_encryption_key() -> bytes:
    ...
    print(
        "WARNING: TF_ANALYZER_SALT_KEY environment variable not set!", file=sys.stderr
    )
    print("=" * 70, file=sys.stderr)
    ...
    print(
        "  - Store this key in a secure location (password manager, CI secrets)",
        file=sys.stderr,
    )
    print("  - DO NOT commit this key to version control", file=sys.stderr)
    print(
        "  - This key will be visible in terminal history and CI logs", file=sys.stderr
    )
    print(
        "  - Without this key, you cannot decrypt/reuse the salt file", file=sys.stderr
    )
    print(
        "  - For CI/CD: Store in GitHub Secrets, GitLab CI variables, etc.",
        file=sys.stderr,
    )
    print("=" * 70, file=sys.stderr)
    ...
    try:
        with open(salt_file, "rb") as file:
            ...
            if len(position_seed) != 32:
                raise ValueError(
                    "Invalid salt file: missing or incomplete position seed"
                )
        ...
        try:
            ...
            print(
                f"  Ensure TF_ANALYZER_SALT_KEY environment variable is set correctly",
                file=sys.stderr,
            )
