...
def load_ignore_config(file_path: Path) -> Dict:
    ...
    if not isinstance(config, dict):
        raise ValueError(f"Ignore configuration must be a JSON object, got {type(config).__name__}")
...
def apply_ignore_config(resource_config: Dict, ignore_rules: Dict, resource_type: str) -> Dict:
    """
...
def get_ignored_attributes(resource_config: Dict, ignore_rules: Dict, resource_type: str) -> Set[str]:
    """
