...
def highlight_char_diff(
    ...
    char_added_class = (
        "char-known-after-apply" if is_known_after_apply else "char-added"
    )
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == "equal":
            ...
            before_parts.append(
                f'<span class="char-removed">{html.escape(before_str[i1:i2])}</span>'
            )
        elif tag == "insert":
    ...
    if strings_identical and should_highlight:
        ...
        for before_line, after_line in zip(before_lines, after_lines):
            if "(changed)" in before_line or "(changed)" in after_line:
                ...
                before_html_lines.append(
                    f'<span class="removed">{html.escape(before_line)}</span>'
                )
        ...
        if len(before_lines) > len(after_lines):
            for line in before_lines[len(after_lines) :]:
                if "(changed)" in line:
                    before_html_lines.append(
                        f'<span class="removed">{html.escape(line)}</span>'
                    )
                else:
                    before_html_lines.append(
                ...
                for idx in range(max_len):
                    if idx < len(before_chunk) and idx < len(after_chunk):
                        ...
                        if (
                            ...
                            before_html_lines.append(
                                f'<span class="removed">{before_highlighted}</span>'
                            )
                            after_html_lines.append(
