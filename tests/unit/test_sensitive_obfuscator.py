...
import pytest
import hashlib
from src.security.sensitive_obfuscator import (
    obfuscate_value,
    get_salt_position,
...
class TestGetSaltPosition:
    ...
    def test_different_seeds_produce_different_positions(self):
        ...
        assert (
            pos1 != pos2 or len(value_bytes) < 2
        )  # Allow equality only for very short values
...
class TestTraverseAndObfuscate:
    ...
    def test_obfuscate_simple_structure(self):
        ...
        sensitive = {"password": True, "username": False}
    ...
    def test_obfuscate_nested_structure(self):
        ...
        data = {"database": {"host": "db.example.com", "password": "secret123"}}
        sensitive = {"database": {"host": False, "password": True}}
    ...
    def test_preserve_non_sensitive_nested_data(self):
        ...
        data = {
            "config": {"public_key": "pk_123", "private_key": "sk_456"},
            "metadata": {"version": "1.0"},
        }
        sensitive = {"config": {"public_key": False, "private_key": True}}
    ...
    def test_deeply_nested_structure(self):
        ...
        data = {"level1": {"level2": {"level3": {"level4": {"secret": "deep_secret"}}}}}
        sensitive = {"level1": {"level2": {"level3": {"level4": {"secret": True}}}}}

        result = traverse_and_obfuscate(data, sensitive, salt, position_seed)

        assert result["level1"]["level2"]["level3"]["level4"]["secret"].startswith(
            "obf_"
        )
