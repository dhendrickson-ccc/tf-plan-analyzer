...
def test_basic_obfuscation(tmp_path):
    ...
    assert (output_file.parent / f"{output_file.name}.salt").exists(), "Salt file not created"
    ...
    assert len(password) == 68, f"Invalid hash length: {len(password)}"  # "obf_" + 64 hex chars
...
def test_multiple_resources(tmp_path):
    ...
    assert db1_password == db2_password, "Identical passwords should produce identical hashes"
...
def test_error_behavior(tmp_path):
    ...
    assert not output_file.exists(), "Output file should not be created when errors occur"
    assert not (output_file.parent / f"{output_file.name}.salt").exists(), "Salt file should not be created on error"
...
def test_deterministic_cross_file(tmp_path):
    ...
    run_obfuscate([str(input2), "--output", str(output2), "--salt-file", str(salt_file)])
    ...
    assert password1 == password2, "Identical sensitive values should produce identical hashes with same salt"
    assert password1.startswith("obf_")
...
def test_drift_detection(tmp_path):
    ...
    run_obfuscate([str(input_prod), "--output", str(output_prod), "--salt-file", str(salt_file)])
    ...
    assert dev_after["password"] != prod_after["password"], "Different values should produce different hashes"
    
    # Same API key should have same hash
    assert dev_after["api_key"] == prod_after["api_key"], "Identical values should produce identical hashes"
